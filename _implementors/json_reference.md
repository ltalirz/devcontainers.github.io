...
| `build.dockerfile` | string |**Required** when using a Dockerfile. The location of a [Dockerfile](https://docs.docker.com/engine/reference/builder/) that defines the contents of the container. The path is relative to the `build.context`, if specified. Otherwise the path is relative to the `devcontainer.json` file. |
